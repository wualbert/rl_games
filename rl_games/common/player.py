...
class BasePlayer(object):
    def __init__(self, params):
        ...
        self.num_policies = (
            1 if isinstance(params["load_path"], str) else len(params["load_path"])
        )
        if self.num_policies > 1:
            assert (
                self.policy_selection_idx is not None
            ), "policy_selection_idx must be specified when loading multiple policies"
            ...
            for n in range(self.max_steps):
                ...
                if done_count > 0:
                    ...
                    if self.print_stats:
                        ...
                        if print_game_res:
                            print(
                                f"reward: {cur_rewards_done:.4} steps: {cur_steps_done:.4} w: {game_res}"
                            )
                        else:
                            print(
                                f"reward: {cur_rewards_done:.4} steps: {cur_steps_done:.4f}"
                            )
